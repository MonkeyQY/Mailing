...
jwt_token = os.getenv("JWT_TOKEN_FOR_OUTSIDE_SERVICE", "sfhwdfgesfg")

# email and password for send email
my_email = os.getenv("MY_EMAIL", "email")
my_email_password = os.getenv("MY_EMAIL_PASSWORD", "password")

# smtp server
smtp_server_gmail = os.getenv("SMTP_SERVER_GMAIL", "smtp.gmail.com")
smtp_port_gmail = int(os.getenv("SMTP_PORT_GMAIL", 465))

# List users for send statistics
list_users_for_send_statistics = list(
    os.getenv("LIST_USERS_FOR_SEND_STATISTICS", [f"{my_email}"])
)
