...
class SendMail:
    ...
    @classmethod
    async def start_scheduler(cls, mailing: Mailing) -> None:
        log.info(f"Start scheduler for mailing {mailing.id}")
        await cls(mailing)._start_sends()

    async def _get_clients(
        self, clients_repository: ClientRepository = get_client_repository()
    ) -> list[Client]:
        clients = await clients_repository.get_clients_where_pattern(
            self.mailing.filter
        )
        clients = [
            client
    ...
    async def _start_sends(self) -> None:
    ...
    async def _send_to_client(
        ...
        log.info(
            f"Send message to client {client.id} with response {response},"
    ...
    @staticmethod
    async def stop_scheduler(mailing_id: int) -> None:
