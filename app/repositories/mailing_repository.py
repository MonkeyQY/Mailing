...
class MailingRepository(BaseRepository):
    ...
    async def create(self, mailing: MailingAdd) -> int:
    ...
    async def update(self, mailing: MailingUpdate) -> None:
        query = (
            mailings.update()
            .where(mailings.c.id == mailing.id)
            .values(
                filter=mailing.filter,
                text_message=mailing.text_message,
                time_sending=mailing.time_sending,
    ...
    async def delete(self, mailing: MailingDelete) -> None:
    ...
    async def end_time_mailing(self, mailing_id: int) -> None:
        query = (
            mailings.update()
            .where(mailings.c.id == mailing_id)
            .values(end_time=datetime.datetime.utcnow())
        )
        return await self.database.execute(query=query)
