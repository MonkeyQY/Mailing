...
async def detail_for_the_mailing(mailing: DetailStatistic) -> DetailStatisticResponse:
    log.info(f"Detail for the mailing request received, mailing: {mailing.mailing_id}")

    try:
        statistics = await GetStatisticsForMailing.get_statistics_for_mailing(
            mailing.mailing_id
        )
...
class GetStatisticsForMailing:
    ...
    @classmethod
    async def get_statistics_for_mailing(cls, mailing_id: int) -> dict:
    ...
    async def get_detail_for_the_mailing(self, mailing_id: int) -> dict:
        ...
        messages = await self.message_repository.get_all_messages_for_mailing(
            mailing_id
        )
