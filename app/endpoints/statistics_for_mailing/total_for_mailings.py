...
async def total_for_mailings() -> StatisticMailingResponse:
...
class GetStatistic:
    ...
    async def _get_total_for_mailings(self) -> list[Mailing]:
        return await self.mailing_repository.get_all()

    async def _get_messages_for_the_mailing(self, mailing_id: int) -> list[Message]:
        list_messages = await self.message_repository.get_all_messages_for_mailing(
            mailing_id
        )
    ...
    async def _group_by_sending_status(self, list_messages: list[Message]) -> dict:
        ...
        return await self._from_list_messages_to_dict(
            list_sending_status_false, list_sending_status_true
        )

    @staticmethod
    async def _from_list_messages_to_dict(
        list_sending_status_false: list, list_sending_status_true: list
    ) -> dict:
        dict_of_messages = {
